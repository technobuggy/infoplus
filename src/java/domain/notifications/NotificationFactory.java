...
public final class NotificationFactory {
     ...
     */
    public static AbstractNotification createNotification(final int id,final Type type,
            final Adres adres, final String description, final int visible, final String afbeelding) {
        ...
        tempNotification.setNotificatieID(id);
        tempNotification.setAdres(adres);
        tempNotification.setDescription(description);
        tempNotification.setVisible(visible);
        tempNotification.setAfbeelding(afbeelding);
        return tempNotification;
    }
    public static Type stringToType(String type){
        type = type.toLowerCase();
        Type actType = Type.DANGEROUSSITUATION;
        if (type.equals("dangeroussituation")) {
            actType = Type.DANGEROUSSITUATION;
        } else if (type.equals("litter")) {
            actType = Type.LITTER;
        } else if (type.equals("roadsurface")) {
            actType = Type.ROADSURFACE;
        } else if (type.equals("neighbourhoodevent")) {
           actType = Type.NEIGHBOURHOODEVENT;
        } else if (type.equals("neighbourhoodaction")) {
            actType = Type.NEIGHBOURHOODACTION;
        }
